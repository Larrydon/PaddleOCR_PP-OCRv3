...
三種數據集的區別為了讓模型訓練更科學(以下的比例，比較像是從頭訓練，若是基於微調的優化，要靠經驗去調整lr和比例)<br>
，通常我們會將數據分為以下三類：<br>
...
>	評估/驗證集的作用是「照妖鏡」<br>
	...
	絕對不能出現在 train_list.txt 裡（這叫數據洩露，會導致評估失真），也不可用合成照片。<br>
	train_list 和 val_list 中的真實照片比例可採用 8:2，對小資料集較友善，較容易收斂<br>
...
=>	我會先使用 rec_carplate_train_gpu.yml 跑第一輪訓練看看結果好不好<br>
	...
	也很固定長度比例(最多碼也就8碼，為了保險起見，改成10碼) 所以<br>
	RecConAug        # 隨機拼接增強，增加模型處理長短車牌的能力<br>
	可以不用開啟，還可以減少拼接後亂辨識的問題。<br>
	棄用蒸餾訓練，改成單純CTC訓練，時間縮短，模型還更小(rec_carplate_train_gpu_CTC.yml)<br>
